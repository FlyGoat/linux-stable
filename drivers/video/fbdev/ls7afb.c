...
static struct fb_var_screeninfo ls7a_fb_default __initdata = {
	.xres		= 1280,
	.yres		= 1024,
	.xres_virtual	= 1280,
	.yres_virtual	= 1024,
...
static struct fb_fix_screeninfo ls7a_fb_fix __initdata = {
	.id =		"Virtual FB",
...
static bool ls7a_fb_enable __initdata = 0;	/* disabled by default */
...
static void config_pll(volatile void *pll_base, struct pix_pll *pll_cfg)
{
	...
	/* set sel_pll_out0 0 */
	val = readq(pll_base + LO_OFF);
	val &= ~(1UL << 40);
	writeq(val, pll_base + LO_OFF);
	/* pll_pd 1 */
	val = readq(pll_base + LO_OFF);
	val |= (1UL << 45);
	writeq(val, pll_base + LO_OFF);
 ...
 */
static void ls7a_enable_cursor(int mode, unsigned long base)
{
...
static void ls7a_fb_address_init(void)
{
         ...
         if(pdev){
                 ...
                 ls7afb_dma = pci_resource_start(pdev, 2);
                 ls7afb_mem = LS7A_HT1_BASE|ls7afb_dma;

                 ls7a_cursor_mem = ls7afb_mem | LS7AFB_MASK;
                 ls7a_cursor_dma = ls7afb_dma | LS7AFB_MASK;
                 ls7a_fb_mem     = ls7afb_mem;
                 ls7a_phy_addr   = ls7afb_mem & LS7AFB_GPU_MASK;
                 ls7a_fb_dma     = ls7afb_dma;
